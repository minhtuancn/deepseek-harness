...
The production Web runner needs built package and frontend artifacts (`pnpm run build`). It serves `http://127.0.0.1:3080` by default. The CLI supports `--host 0.0.0.0` to bind to all network interfaces; `--trusted-host` adds named authorities accepted by the `/api` browser-trust fence.
